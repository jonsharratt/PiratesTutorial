...
class PlayerLifeCycleManager(val world: AppWorld, logger: Logger, val lifecycle: WorldAppLifecycle) extends WorldApp {
  ...
  private def addEntity(userId: String): Unit = {
    val playerEntityId = world.entities.spawnEntity(Player(engineId = userId))
